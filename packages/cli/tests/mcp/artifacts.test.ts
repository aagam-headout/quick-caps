...
describe('resolveArtifactRoot', () => {
  it('defaults to a quickcaps-mcp-artifacts dir under the OS tmpdir', () => {
    expect(resolveArtifactRoot({})).toBe(join(tmpdir(), 'quickcaps-mcp-artifacts'));
  });

  it('honors QUICKCAPS_MCP_ARTIFACT_ROOT when set', () => {
    expect(resolveArtifactRoot({ QUICKCAPS_MCP_ARTIFACT_ROOT: '/custom/root' })).toBe(
      '/custom/root',
    );
  });
});
...
describe('resolveRetentionMs', () => {
  ...
  it('honors QUICKCAPS_MCP_ARTIFACT_RETENTION_MS when a positive number', () => {
    expect(resolveRetentionMs({ QUICKCAPS_MCP_ARTIFACT_RETENTION_MS: '1000' })).toBe(1000);
  });

  it('falls back to the default on garbage input', () => {
    expect(resolveRetentionMs({ QUICKCAPS_MCP_ARTIFACT_RETENTION_MS: 'nope' })).toBe(
      24 * 60 * 60 * 1000,
    );
    expect(resolveRetentionMs({ QUICKCAPS_MCP_ARTIFACT_RETENTION_MS: '-5' })).toBe(
      24 * 60 * 60 * 1000,
    );
  });
});
...
describe('ensureArtifactRoot', () => {
  it('creates the directory recursively if missing', async () => {
    const root = join(await mkdtemp(join(tmpdir(), 'quickcaps-ensure-')), 'nested', 'dir');
